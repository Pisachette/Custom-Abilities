...
# BOMB FABRICATION
    # check for offhand item, and run function if item is in offhand
        execute as @a[team=Dinomyte08] if score @s dino.explosionTimer matches 60..61 if items entity @s weapon.offhand * run function dino:abilities/ability_boomstick_fabricate
    # bomb throw mechanics
        execute as @a[team=Dinomyte08] run function dino:abilities/ability_boomstick_throw

# EXPLODE-ON-DEATH
    # reset death check when player spawns back in to prevent explode looping
